...
from .appearance import (
    ...
    XYZ_to_RLAB)
from .difference import delta_E
...
for access_change in _API_CHANGES['Future Access Change']:
    old_access, new_access = access_change
    _API_CHANGES[old_access.split('.')[-1]] = (FutureAccessChange(
        old_access, new_access))
_API_CHANGES.pop('Future Access Change')

for access_remove in _API_CHANGES['Future Access Remove']:
    name, access = access_remove
    _API_CHANGES[name.split('.')[-1]] = (FutureAccessRemove(name, access))
_API_CHANGES.pop('Future Access Remove')

for removed in _API_CHANGES['Removed']:
    _API_CHANGES[removed.split('.')[-1]] = (Removed(removed))
_API_CHANGES.pop('Removed')
...
sys.modules['colour'] = colour(sys.modules['colour'], _API_CHANGES)
